...
var f4Posterior = new TextFormat("Helvetica-Light", 20 * pixel_ratio, 0x000000, true, false, "left");
...
function Start(fun, ss, tt, rn, counterbalance, posit_ix_, tr_count) {
    ...
    for (var i = 0; i < 9; i++) {
        ...
        for (var j = 1; j < points.rhs.length; j++) { s.graphics.lineTo(points.rhs[j].x * half_ratio, points.rhs[j].y * half_ratio); }
        s.graphics.endFill();

        //Left hand side
        s.graphics.beginFill(col, .5);
        s.graphics.moveTo(points.lhs[0].x * half_ratio, points.lhs[0].y * half_ratio);
        for (var j = 1; j < points.rhs.length; j++) { s.graphics.lineTo(points.lhs[j].x * half_ratio, points.lhs[j].y * half_ratio); }
        s.graphics.endFill();
...
function NextTest() {
    ...
    if (test_count < start_state.length) {
        ...
        for (var i = 0; i < start_state[test_count].ids.length; i++) {
            ...
            var description = {
                colour: start_state[test_count].colours[i], xpos: start_state[test_count].xpos[i], ypos: start_state[test_count].ypos[i],
        ...
        t1.y = stage.stageHeight * t1YPosition;
...
function AddPiece(e, description) {
    ...
    if (pieces[piece_type] < max_pieces[piece_type]) {
        ...
        body.SetUserData({
            type: "piece", size: [1, 2, 3][piece_type % 3],
...
function TestDevice(e) {
    ...
    if (check == true) {
        ...
        trialdata.push({
            ids: ids, colours: colours, sizes: sizes, xpos: xposs, ypos: yposs, rotations: rotations, orientations: orientations,
